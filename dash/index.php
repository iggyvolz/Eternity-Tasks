...
<body>
	...
	<div id="container">
		...
		<p>Welcome to the brand new tasks dashboard, now you can do more with Eterntiy Tasks, the spark that burned an X on the Eternity!<br/><br/>The below table displays the users by EP descending</p>
                <?php
         //VARIABLES
         $result = $db->query("SELECT * FROM users WHERE rank='Eternity Team' ORDER BY EP DESC");
...
{
		echo "<tr>";
		echo 	"<td><a href=\"http://eternityinc-official.com/users/$row[username]\">$row[username]</a></td>";
		echo 	"<td>$row[banned]</td>";
		echo 	"<td>$row[EP]</td>";
                echo    "<td><iframe src=\"http://api.eternityinc-official.com/status-image/$row[username]\" frameborder=\"0\" width=\"50px\" height=\"50px\" \"></iframe></td>";
